    ...
    if (theme === "classic") {
        for (let i = 0; i < paths.length; i++) {
            if (document.querySelectorAll('.qr-home path')[i].getAttribute('fill') === dark) {
                ...
                document.querySelectorAll('.qr-home path')[i].setAttribute('fill', '#F2EADF')
            }
        }
        document.documentElement.style.setProperty('--clr-neutral-900', '#262523');
        document.documentElement.style.setProperty('--clr-neutral-100', '#F2EADF');
    }
    if (theme === "modern") {
        for (let i = 0; i < paths.length; i++) {
            if (document.querySelectorAll('.qr-home path')[i].getAttribute('fill') === dark) {
                document.querySelectorAll('.qr-home path')[i].setAttribute('fill', '#FFF')
            } else {
                document.querySelectorAll('.qr-home path')[i].setAttribute('fill', '#000')
            }
        }
        document.documentElement.style.setProperty('--clr-neutral-900', '#FFF');
        document.documentElement.style.setProperty('--clr-neutral-100', '#000');
    }
    if (theme === "unknown") {
